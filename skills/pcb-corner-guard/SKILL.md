...
description: 按尺寸生成可打印的 PCB 护角、三面护角、U 形、四周封闭或圆形封闭盖板，以及带可选圆角和倒角的圆形、矩形、方形实心板，输出 STL、源文件与三维预览。
---
...
`--type roundcover --diameter 100 --height 15 --base 3 --wall 3 --opening_chamfer 1 --outer_radius 2 --inner_radius 2 --out <输出目录>`。

内径为直壁段的净直径，内高从内腔中央平面量到开口；不自动增加装配间隙。外径 D+2S，总高 H+T。开口朝上展示，使用时翻转盖合。

开口倒角 C 是开口内缘 45° 导入斜面（轴向与径向各 C）；外缘圆角 R外 位于封闭顶板和外侧壁交界，切除材料；内部圆角 R内 位于内腔底面和侧壁交界，增加材料并占用内腔边角。三个修饰参数默认 0，不自动添加。

要求 C<S，R外≤min(T,S)，R内<D/2，R内+C<H；不静默缩小非法参数。圆周与圆角各自最大弦高误差 0.025 mm。

输出 cover_single.stl、cover_parametric.scad、preview.html、parameters.json 和打印说明；外缘圆角底部可能需要支撑，先切片检查并试配。此类型不是实心圆形板。
